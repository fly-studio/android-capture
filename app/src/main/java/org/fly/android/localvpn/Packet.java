...
public class Packet
{
    ...
    public static final int MUTE_SIZE = 1496;
    ...
    public Packet(ByteBuffer buffer, TYPE type) throws UnknownHostException {
    ...
    {
        return ip4Header.headerLength + (isTCP() ? tcpHeader.headerLength : 8);
    }
    ...
    public void generateUDPBuffer(ByteBuffer buffer, int payloadSize)
    {
        buffer.position(0);
        fillHeader(buffer);
        backingBuffer = buffer;
    ...
    public static class IP4Header
    {
        ...
        public byte version = 0; //0: 版本号（4bits）
        public byte IHL = 0; // 首部长度（4bits）
        public int headerLength;
        public short typeOfService = 1; //1：服务类型偏移
        public int totalLength = 2; //2：总长度偏移

        public int identificationAndFlagsAndFragmentOffset = 4; //4：16位标识符偏移

        public short TTL = 8; //8：生存时间偏移
        private short protocolNum;
        public TransportProtocol protocol;
        public int headerChecksum = 10; //10：首部校验和偏移
    ...
    public static class TCPHeader
    {
        ...
        public int sourcePort = 0; // 16位 源端口
        public int destinationPort = 2; // 16位 目的端口

        public long sequenceNumber = 4; //32位 序列号
        public long acknowledgementNumber = 8; //32位 确认号

        public byte dataOffsetAndReserved = 12; //4位 首部长度 + 4位 保留位
        public int headerLength = 12; // dataOffsetAndReserved的 高4位
        public byte flags = 13; //2位 保留字 + 6位 标志位
        public int window = 14; //16位 窗口大小

        public int checksum; //16位 校验和
        public int urgentPointer = 18; //16位 紧急偏移量
    ...
    public static class UDPHeader
    {
        ...
        public int sourcePort = 0; // 源端口
        public int destinationPort = 2; //目的端口

        public int length = 4; //数据报长度
        public int checksum = 6; //校验和
